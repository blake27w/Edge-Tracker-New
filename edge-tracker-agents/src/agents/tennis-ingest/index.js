...
function normalize(key, ev, snapshots, movements, lsRows) {
  ...
  for (const bm of ev.bookmakers || []) {
    ...
    for (const mk of bm.markets || []) {
      for (const oc of mk.outcomes || []) {
        ...
        game.books[bm.key].markets[`${mk.key}:${oc.name}`] = { line: oc.point ?? null, price };
        snapshots.push({ game_id: ev.id, tournament: key, p1, p2, book: bm.key, market: mk.key, side: oc.name, line: oc.point ?? null, price, fetched_at: now });
        // Mirror into the unified line_snapshots so the CLV tracker (which only
        // reads that table) can compute tennis CLV like every other sport.
        lsRows.push({ sport: 'TENNIS', game_id: ev.id, commence_time: ev.commence_time, home: p1, away: p2, book: bm.key, market: mk.key, side: oc.name, line: oc.point ?? null, price, fetched_at: now });
...
async function run() {
  ...
  const games = [], snapshots = [], movements = [], lsRows = [];
  for (const key of keys) {
    try {
      const evs = await fetchMatches(key);
      for (const ev of evs) games.push(normalize(key, ev, snapshots, movements, lsRows));
  ...
  if (lsRows.length) { try { await db.insert('line_snapshots', lsRows); } catch (e) { logger.warn('tennis-ingest', `line_snapshots: ${e.message}`); } }
