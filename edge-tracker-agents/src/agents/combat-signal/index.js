...
// NOT project fights. Emits OBSERVATIONAL UFC plays (sport=UFC,
...
async function run() {
  const games = getGames().filter((g) => g.sport === 'UFC');
  if (!games.length) { setCombatPlays([]); return { summary: 'no UFC card' }; }
  ...
  const addSig = (game_id, side, score, sig) => {
    ...
    if (!plays[k]) plays[k] = { sport: 'UFC', game_id, matchup: `${g.away} vs ${g.home}`, commence_time: g.commence_time, market: 'ml', side, score: 0, t1_count: 0, signals: [] };
  ...
  try {
    ...
    const news = await db.select('combat_news', '*', { gte: { detected_at: since }, match: { sport: 'UFC' } });
    for (const n of news) {
  ...
  for (const s of (getIntel('sharp') || [])) {
    if (s.sport === 'UFC' && s.market === 'h2h') addSig(s.game_id, s.side, Math.min(82, 60 + Math.round((s.strength || 0) / 4)), { tier: 1, id: 'steam', label: s.detail || 'sharp steam' });
  }

  // 3. +EV (T2) — soft price vs the field.
  for (const e of (getEvPlays() || [])) {
    if (e.sport === 'UFC' && e.market === 'ml') addSig(e.game_id, e.side, 72, { tier: 2, id: 'ev', label: `+${e.ev_pct}% vs fair @ ${e.book}` });
  }
  ...
  if (fresh.length) {
    ...
    try {
      await db.insert('monitor_scores', fresh.map((p) => ({
        sport: 'UFC', game_id: p.game_id, matchup: p.matchup, market: 'ml', side: p.side, line: null,
  ...
  return { summary: `${list.length} UFC observational plays (${fresh.length} new) · validation-gated`, data: { plays: list.length } };
}
